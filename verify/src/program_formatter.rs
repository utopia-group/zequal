...
use program_structure::ast::{Access, AssignOp, Expression, ExpressionInfixOpcode, ExpressionPrefixOpcode, SignalType, Statement, TagList, VariableType};
...
pub fn format_statement(num_levels: usize, stmt: &Statement) -> String {
    match stmt {
        ...
        Statement::Declaration { name,   xtype, .. } =>
            format!("{}{} {};",
                "  ".repeat(num_levels),
...
pub fn format_program(program: &ProgramArchive) -> String {
    ...
    if let Call {meta: _,  id, args } = program.get_main_expression() {
